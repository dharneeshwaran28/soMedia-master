...
def followers(request):
    ...
    users_followed = request.user.followers.all().exclude(is_superuser=True)
    
    # get_users not followed and exclude current_user from the list
    unfollowed_users = User.objects.exclude(id__in=users_followed).exclude(id=request.user.id).exclude(is_superuser=True)
